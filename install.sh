...
echo -e "${BLUE}Installing slash commands for workplace communication...${NC}"
echo ""
...
echo -e "  ${BOLD}/polish${NC}              Make a rough message land better (tone flags: casual, formal, executive, urgent, technical)"
echo -e "  ${BOLD}/shorten${NC}             Cut a message by 50%+ without losing decisions, deadlines, or action items"
echo -e "  ${BOLD}/tone-shift${NC}          Rewrite in a different tone (assertive, diplomatic, casual, executive, empathetic, technical)"
echo ""
echo -e "  ${BOLD}Respond -- handle messages from others:${NC}"
echo -e "  ${BOLD}/read-the-room${NC}       Decode what someone's message actually means and get response strategies"
echo -e "  ${BOLD}/decline-politely${NC}    Say no clearly and kindly without hedging or fake alternatives"
echo -e "  ${BOLD}/follow-up${NC}           Write a follow-up that's direct without being passive-aggressive"
...
echo -e "  ${BOLD}/escalation${NC}          Turn a rough problem into a structured escalation email"
echo -e "  ${BOLD}/thread-summary${NC}      Distill a long thread into decisions, owners, and open questions"
...
echo -e "  - All commands preserve your voice. The output should sound like you on your best day."
